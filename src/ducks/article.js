...
export const createArticle = ({ title, content }) => async (dispatch) => {
  ...
  dispatch(articleCreating()); // dispatch를 시작함
  try {
    ...
    const contentPromise = firebase.database().ref(`content/${articleRef.key}`).set(content);
    await Promise.all([articleRef, contentPromise]);
    dispatch(articleSuccess()); // dispatch를 끝남
  } catch (e) { // error객체
    dispatch(articleError(`알수없는 에러가 발생했습니다. 다시 시도해 주세요.: ${e.errorMessage}`));
  }
};
